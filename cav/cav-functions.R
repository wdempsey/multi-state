compute.pdf <- function(trans.matrix,beta.matrix, rho) {
  f2 <- function(x) {
    ...
    for (i in 1:length(r.vector)) {
      log.total = log.total + r.vector[i]*beta.vector[i] * log(1-x) + d.vector[i] * log( 1 - x^(beta.vector[i])) 
    }
...
q.split <- function(r.vector, d.vector, beta.vector, rho, tol = 0.000001) {
  temp = compute.pdf(r.vector, d.vector, beta.vector, rho)
...
compute.normalizing <- function(n.vector, beta.vector, rho) {
  f2 <- function(x) {
    ...
    for (i in 1:length(n.vector)) {
      log.total = log.total + log( 1- x^(sum(n.vector*beta.vector)) )
    }
    return( exp(log.total) )
  }
  return(f2)
...
lik.component <- function(params, n.vector, d.vector, isdeathtime, window.time, rho) {
  r.vector = n.vector-d.vector
  nu = params[1:2]; beta.alive = c(1,params[3:5]); beta.dead = c(1,params[6:7])
